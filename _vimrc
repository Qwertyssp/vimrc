...
auto BufNewFile *.[ch],*.cpp,*.sh,*java,*.php,*.py,*.cc exec ":call SetTitle()"
"新建文件后，自动定位到文件末尾
autocmd BufNewFile * normal G
"写入.c, .h.cpp, .sh , .java, .php, .py文件自动更新ctags
autocmd BufWrite *.[ch],*.cpp,*.sh,*.java,*.php,*.py,*.cc exec ":!ctags -R *"
...
func SetTitle() 
    ...
    if &filetype == 'c' 
        ...
        call append(line(".")+1, "    > Author       : shang") 
        call append(line(".")+2, "    > Mail         : shangshipei@163.com ") 
        call append(line(".")+3, "    > Description  : shang ")
        call append(line(".")+4, "    > Created Time : ".strftime("%c")) 
        call append(line(".")+5, "    > Rev          : 0.1")
        call append(line(".")+6, " ------------------------------------------------------------------------*/") 
        call append(line(".")+7, " ")
        call append(line(".")+8, " /*------------------------------------------------------------------------") 
        call append(line(".")+9, "    > Revision log :") 
        call append(line(".")+10, " " )
        call append(line(".")+11, "    > Created by shang, ".strftime("%c")) 
        call append(line(".")+12, " ------------------------------------------------------------------------*/") 
        call append(line(".")+13, "")
    endif
endfunc
...
"定义函数SetFuncTitle,自动插入函数说明
func SetFuncTitle()
    call append(line("."), "/*-------------------------------------------------------------------------") 
    call append(line(".")+1, "    > Functions    : none") 
    call append(line(".")+2, "    > Description  : none") 
    call append(line(".")+3, "    > Input        : none ")
    call append(line(".")+4, "    > Output       : none ")
...
map <F4> :call SetTitle()<CR> G
map <F3> :call SetFuncTitle()<CR> $
map <F2> :call SetWireComment()<CR> $
map <F6> :Tlist<CR>
map <F8> :!ctags -R *<CR>
map <F9> :!splint % <CR>
